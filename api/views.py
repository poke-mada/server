...
class TrainerViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=False, methods=['post'])
    def register_overlay_data(self, request, *args, **kwargs):
        from websocket.sockets import OverlayConsumer
        deaths = request.data.get('deaths', 0)
    ...
    def get_economy(self, request, *args, **kwargs):
        ...
        return Response(current_profile.economy or 0, status=status.HTTP_200_OK)
    ...
    def list_trainers(self, request, *args, **kwargs):
        ...
        trainer_ids = MastersProfile.objects.filter(is_pro=is_pro, profile_type=MastersProfile.TRAINER,
                                                    is_tester=is_tester, user__is_active=True,
    ...
    def box(self, request, pk=None, *args, **kwargs):
        ...
        box_serializer = TrainerBoxSerializer(box, context=dict(request=request), read_only=True)
    ...
    def list_players(self, request, *args, **kwargs):
        ...
        profiles = MastersProfile.objects.filter(is_pro=current_profile.is_pro, user__is_active=True,
...
def box_saver(boxes, profile: MastersProfile):
    ...
    boxes_to_delete = TrainerBox.objects.filter(trainer=trainer)
...
class NewsletterViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if profile.profile_type == MastersProfile.ADMIN:
            ...
            if profile.is_pro:
                queryset = self.get_queryset().filter(Q(target_method=Newsletter.PROS) | Q(target_method=Newsletter.ALL))
            else:
                queryset = self.get_queryset().filter(Q(target_method=Newsletter.NOOBS) | Q(target_method=Newsletter.ALL))
